...
3. ~~**Dos fuentes de verdad para los equipos.** `user.pokemons` y `draft.picks` se sincronizan a mano en cada operación. Hacer que `draft.picks` sea la única fuente y eliminar `UserEntity.pokemons`.~~ ✅ PR #PENDING (`draft.picks` como única fuente; corrige de paso que un draft cancelado dejaba Pokémon "cogidos" en la banca).
