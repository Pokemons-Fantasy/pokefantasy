...
2. **Comprar Pokémon de la banca con monedas** — players can spend coins to acquire an unclaimed Pokémon from the bench (one that was never drafted). Price follows the existing tier pricing (`priceTierS/A/B/C/D` from `LeagueSettings`). Must respect the steal/swap window rules (or define its own window). Buyer's coin balance decreases; Pokémon moves from bench to the buyer's team; `DraftEntity.picks` updated so `TeamsPage` reflects the change.
3. **Popup unificado en Pokémon rival** — al hacer click en un Pokémon de otro jugador, mostrar un modal con dos opciones: "Robar" (si la ventana de robo está abierta y el jugador puede permitírselo) y "Proponer intercambio" (siempre disponible tras el draft). Actualmente cuando la ventana de robo está abierta se salta directamente al `StealModal` sin dar la opción de trade.
