...
- **App icons + splash screen** — Asset maestro 1024×1024px + `@capacitor/splash-screen`. Necesario antes de distribuir el APK.
- **Deep links (invite)** — Registrar intent-filter en `android/AndroidManifest.xml` para que los links `/invite/:token` abran la app. Requiere scheme `pokefantasy://` en `capacitor.config.ts`.
- **Status bar color** — `@capacitor/status-bar` para adaptar el color al tema dark/light (safe area ya está resuelta por CSS; esto es para el color de iconos de la barra).
- **Haptics** — `@capacitor/haptics` en pick de draft, confirmar robo, aceptar trade.
